...
from utils.loss import ComputeLoss, ComputeNWDLoss, ComputeLossOTA, ComputeLossAuxOTA
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    global fi, lr, mloss, epoch, val_loader, ckpt, loggers
    ...
    if RANK in [-1, 0]:
        val_loader = create_dataloader(val_path, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls, hyp=hyp,
                                       cache=None if noval else opt.cache, rect=True, workers=workers * 2, pad=0.5,
    ...
    if RANK in [-1, 0]:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = val.run(data_dict, batch_size=batch_size // WORLD_SIZE * 2, imgsz=imgsz,
                                            ...
                                            save_json=is_coco, verbose=True, callbacks=callbacks,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cache', type=str, nargs='?', default=True, const='ram',
