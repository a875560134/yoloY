...
class VFocalLoss(nn.Module):
    def __init__(self, loss_fcn, alpha=0.75, gamma=2.0, use_sigmoid=True):
...
class GFocalLoss(nn.Module):
    def __init__(self, loss_fcn, beta=2.0, use_sigmoid=True):
    ...
    def forward(self, pred, true):
        ...
        pred_prob = torch.sigmoid(pred)
        scale_factor = (pred_prob - true).abs().pow(self.beta)
...
class ComputeLossBinOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        wh_bin_sigmoid = SigmoidBin(bin_count=self.bin_count, max=4.0, use_loss_regression=False).to(device)
