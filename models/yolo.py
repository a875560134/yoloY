            ...
            if type(m) is RepVGGBlock:
                if hasattr(m, 'rbr_1x1'):
                    ...
                    rbr_reparam = nn.Conv2d(in_channels=m.rbr_dense.conv.in_channels,
                                            ...
                                            kernel_size=m.rbr_dense.conv.kernel_size, stride=m.rbr_dense.conv.stride,
                                            padding=m.rbr_dense.conv.padding, dilation=m.rbr_dense.conv.dilation,
                                            groups=m.rbr_dense.conv.groups)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, CrossConv, DWConv, Conv6, DepthWiseConv, PointWiseConv, ConvSig, ConvSqu, RepConv, XBNConv, RobustConv, RobustConv2,
                 ...
                 RepRes, RepResCSPA, RepResCSPB, RepResCSPC, NAMAttention, GAMAttention,
                 RepBottleneckCSPA, RepBottleneckCSPB, RepBottleneckCSPC,
                 C3, C3TR, C3Ghost, C3HB, C3RFEM, C3STR, C3x, C3GC, C3SPP, C3C2,
                 GhostCSPA, GhostCSPB, GhostCSPC, DownC, SPPCSPC, GhostSPPCSPC, GhostBottleneck,
                 STCSPA, STCSPB, STCSPC, SPPCSP, ST2CSPA, ST2CSPB, ST2CSPC, BoT3,
                 HorBlock, CNeB, CoT3, CoT, ConvNextBlock, RepVGGBlockv6, VoVCSP,
                 BottleneckCSPTR, BottleneckCSP2TR, SPPCSPTR, ImplicitA, ImplicitM, ACmix,
                 CBAM, ResBlock_CBAM, CA, Ghost, RepResX, Res, ResX, Bottleneck,
                 BasicRFB, SPPCSPC_group, RepBottleneck, GhostSPPFCSPC,
                 CBH, ES_Bottleneck, SwinTransformerBlock, SwinTransformer2Block, MultiSEAM, SEAM,
                 Stem, MixConv2d, Focus, Involution, nn.ConvTranspose2d, DWConvTranspose2d,
                 DWConvblock, RepVGGBlock, LC_Block, Dense, conv_bn_relu_maxpool,
                 Shuffle_Block, stem, mobilev3_bneck, conv_bn_hswish, MobileNetV3_InvertedResidual,
                 ShuffleNetV2_Model, Conv_maxpool
                 ]:
            c1, c2 = ch[f], args[0]
            ...
            if m in [BottleneckCSP, BottleneckCSPA, BottleneckCSPB, BottleneckCSPC, BottleneckCSP2, BottleneckCSPF,
                     ...
                     BottleneckCSP2TR, CTR3, C2f, GhostSPPCSPC, SPPFCSPC, GhostSPPFCSPC,
                     BottleneckCSP2SAM, InvolutionBottleneck,
                     ResCSPA, ResCSPB, ResCSPC, ResXCSPA, ResXCSPB, ResXCSPC, RepResXCSPA, RepResXCSPB, RepResXCSPC,
                     RepResCSPA, RepResCSPB, RepResCSPC, GhostCSPA, GhostCSPB, GhostCSPC, GhostSPPCSPC, GhostBottleneck,
                     RepBottleneckCSPA, RepBottleneckCSPB, RepBottleneckCSPC,
                     STCSPA, STCSPB, STCSPC, SPPCSP, ST2CSPA, ST2CSPB, ST2CSPC, SPPCSPC,
                     C3, C3TR, C3x, SPPCSPTR, ConvNextBlock, ImplicitA, ImplicitM,
                     C3Ghost, C3HB, C3RFEM, C3STR, C3GC, C3SPP, C3C2,
                     DownC, BoT3, HorBlock, CNeB, CoT3, CoT, RepVGGBlockv6, VoVCSP,
                     ]:
            ...
            c2 = ch[f] * 4
        elif m in [S2Attention, CrissCrossAttention, ShuffleAttention, SEAttention, SimAM, SKAttention]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)
            args = [c1, *args[1:]]
            ...
            c2 = 4 * ch[f]
        elif m in [Detect, ASFF_Detect, IDetect, IBin, MT, IAuxDetect, DetectX, DetectYoloX, Decoupled_Detect]:
            ...
            args = args[1:]
        # torchvision
        elif m in [RegNet1, RegNet2, RegNet3, Efficient1, Efficient2, Efficient3, MobileNet1, MobileNet2, MobileNet3]:
            c2 = args[0]
        elif m is Expand:  # no
            c2 = ch[f] // args[0] ** 2
        else:
