...
class DetectX(nn.Module):
    ...
    def __init__(self,
        ...
        for cls_in_channel, reg_in_channel in zip(cls_in_channels, reg_in_channels):
            cls_pred = nn.Conv2d(in_channels=cls_in_channel, out_channels=self.n_anchors * self.num_classes,
                                 kernel_size=1)
            reg_pred = nn.Conv2d(in_channels=reg_in_channel, out_channels=4, kernel_size=1)
            obj_pred = nn.Conv2d(in_channels=reg_in_channel, out_channels=self.n_anchors * 1, kernel_size=1)
    ...
    def _forward(self, xin):
        ...
        for k, (stride_this_level, cls_x, reg_x) in enumerate(zip(self.stride, cls_xs, reg_xs)):
            ...
            if self.training:
                ...
                size = hsize * wsize
                cls_output = cls_output.view(batch_size, -1, size).permute(0, 2,
                                                                           1).contiguous()  # [batch_size, num_classes, hsize*wsize] -> [batch_size, hsize*wsize, num_classes]
                reg_output = reg_output.view(batch_size, 4, size).permute(0, 2,
                                                                          1).contiguous()  # [batch_size, 4, hsize*wsize] -> [batch_size, hsize*wsize, 4]
                obj_output = obj_output.view(batch_size, 1, size).permute(0, 2,
                                                                          1).contiguous()  # [batch_size, 1, hsize*wsize] -> [batch_size, hsize*wsize, 1]
                if self.use_l1:
                    origin_preds.append(reg_output.clone())
                output, grid, xy_shift, expanded_stride, center_ltrb = self.get_output_and_grid(reg_output, hsize,
                                                                                                wsize, k,
                                                                                                stride_this_level,
                                                                                                in_type)
    ...
    def get_assignments(
        ...
        pair_wise_ious = self.bboxes_iou(gt_bboxes_per_image, bboxes_preds_per_image, inplace=True)
        ...
        with autocast(enabled=False):
            pair_wise_cls_loss = F.binary_cross_entropy(cls_preds_, gt_cls_per_image, reduction="none").sum(
                -1)  # [num_gt, fg_count]
    ...
    def dynamic_k_matching(cost, pair_wise_ious, gt_classes, num_gt, fg_mask_inds):
        ...
        pred_ious_this_matching = pair_wise_ious.index_select(1, fg_mask_inboxes_inds).gather(dim=0,
                                                                                              index=matched_gt_inds[
                                                                                                    None,
